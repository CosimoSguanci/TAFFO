...
int main(int argc,char** argv){
  ...
  while(time_span.count()<=seconds){
    ...
    __attribute__((annotate("range 0 499000000"))) double res[N]; // TAFFO:   __attribute__((annotate("range 0 499000000")))

    for(size_t r_=0;r_<N;r_++){
      res[r_] = std::fmod((N * N),modulo);
    }

    __attribute__((annotate("range 0 499000000"))) double temp = res[0];
